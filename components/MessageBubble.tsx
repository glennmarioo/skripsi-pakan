...
const MiniProductCard: React.FC<MiniProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm hover:border-brand-300 transition-colors">
      {product.image_url && (
        <div className="mb-3 w-full h-32 overflow-hidden rounded-lg bg-slate-50 flex items-center justify-center border border-slate-100">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <span className="text-xs text-slate-500 font-medium px-2 py-0.5 bg-slate-50 border border-slate-200 rounded-md">Sisa {product.stock}</span>
      </div>
      <Button
...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
  return (
    ...
    >
      ...
      >
        <div className={`prose prose-sm max-w-none leading-relaxed ${message.role === 'user' ? 'prose-invert text-white' : 'prose-slate'}`}>
